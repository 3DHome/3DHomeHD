...
public class DownloadManager {
    ...
    public synchronized void applyDownload(String productId) {
        ...
        if (info != null) {
            if (MarketUtils.CATEGORY_THEME.equals(info.type)) {
                ...
                Cursor cursor = mContext.getContentResolver().query(ThemeColumns.THEME_LEFT_JOIN_ALL, null, where,
                        null, null);
                    ...
                    if (!isApplyed) {
                        if (unzipPackage()) {
                            ...
                            if (themeInfo != null) {
                                ...
                                List<ModelInfo> boundingModel = themeInfo.getBoundingModel();
                                if (boundingModel != null && boundingModel.size() > 0) {
                                    for (ModelInfo modelInfo : boundingModel) {
                                        HomeManager.getInstance().getModelManager().mModels.put(modelInfo.mName,
                                                modelInfo);
        ...
        private ThemeInfo loadThemeInfo(boolean isApply) {
            try {
                ...
                ThemeInfo config = ThemeInfo.CreateFromXml(mContext, parser, mPluginPackageInfo.product_id,
                        mInstallPath);
                config.mSECameraData.mFov = 29;
                config.mSECameraData.mLocation.set(0, -803, 460);
                config.mBestCameraFov = 29;
                config.mBestCameraLocation = new SEVector3f(0, -803, 460);
    ...
    private void applyDefaultTheme() {
        ...
        String where = ThemeColumns.PRODUCT_ID + "='" + HomeDataBaseHelper.getInstance(mContext).getDefaultThemeID()
                + "'";
        Cursor cursor = mContext.getContentResolver().query(ThemeColumns.THEME_LEFT_JOIN_ALL, null, where, null, null);
