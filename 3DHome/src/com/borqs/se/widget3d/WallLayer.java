...
public class WallLayer extends VesselLayer {
    ...
    public WallLayer(HomeScene scene, VesselObject vesselObject) {
        ...
        if (scene.getHomeSceneInfo().islandscape()) {
            mIsLandscapeTheme = true;
        } else {
            mIsLandscapeTheme = false;
        }
        mVirtualWallRadius = getHomeSceneInfo().mWallRadius * 0.8f;
        mSkyRadius = getHomeSceneInfo().mSkyRadius * 0.6f;
    }
    ...
    public boolean setOnLayerModel(NormalObject onMoveObject, boolean onLayerModel) {
        ...
        if (onLayerModel) {
            if (null == mAllLayer) {
                ...
                /* moved & merge to HomeScene.getAllWallCellLayerList()
            mAllLayer = new ArrayList<WallCellLayer>();
            for (int i = 0; i < getHomeSceneInfo().mWallNum; i++) {
                Wall wall = mHouse.getWall(i);
                if (wall != null) {
                    mAllLayer.add((WallCellLayer) (wall.getVesselLayer()));
                }
            */
            }
...
//            getOnMoveObject().setUserTransParas();
    ...
    public void setMovePoint(int touchX, int touchY) {
        ...
        mRealLocation = rayCrossWall(ray, getHomeSceneInfo().mWallRadius).mTranslate;
    ...
    private SETransParas getObjectTransParas(SERay ray) {
        ...
        int minZ = (int) (getOnMoveObject().getObjectSlot().mSpanY * getHomeSceneInfo().mCellHeight / 2);
        int maxZ = (int) (getHomeSceneInfo().mWallHeight - minZ);
    ...
    private SETransParas rayCrossWall(SERay ray, float wallRadius) {
        ...
        float faceAngle = 360 / getHomeSceneInfo().mWallNum;
