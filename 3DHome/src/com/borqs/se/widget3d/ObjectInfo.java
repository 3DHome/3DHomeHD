...
//    public static final String WALL_SHELF = "WallShelf";
    public static final String WALL_SHELF = "jiazi";
    ...
    /**
     * 物体不做任何平移，直接加到场景后的最小点，以及最大点。
     * 通过这两个点能够大概知道物体中心点位置以及物体的大小
     */
    ...
    public NormalObject CreateNormalObject(HomeScene scene) {
        ...
        if (mClassName == null) {
            ...
            } else if ("IconBox".equals(mType)) {
                obj = new IconBox(scene, mName, mIndex);
            ...
            } else if (isBottomDecoratorType(mType)) {
                obj = new TrialBottomDecorator(scene, mName, mIndex);
    ...
    public boolean isAppUserUpdate() {
        if (!TextUtils.isEmpty(mType)) {
            if (mType.equals("App")) {
                if (!TextUtils.isEmpty(mDisplayName)) { return true; }
                if (mShortcutIcon != null) { return true; } // TODO: recover?
            }
        }
        return false;
    }
    ...
    public static class ObjectSlot {
        ...
        public void union(ObjectSlot slot) {
            int left = Math.min(left(), slot.left());
            int right = Math.max(right(), slot.right());
            mStartX = left;
            mSpanX = right - left;
            int top = Math.min(top(), slot.top());
            int bottom = Math.max(bottom(), slot.bottom());
            mStartY = top;
            mSpanY = bottom - top;
        }

        public boolean contain(ObjectSlot slot) {
            if (left() <= slot.left() && right() >= slot.right()
    ...
    public static boolean isBottomDecoratorType(String type) {
        if (!TextUtils.isEmpty(type) &&
                type.toLowerCase().startsWith(WALL_SHELF)) {
            return true;
        }
        return false;
    }

    // currently, only shelf is design to be a valid decorator on the bottom of
    // wall cells.
    // todo: improve the design and move such hard conf into model conf of the object.
    public static boolean isBottomDecoratorType(NormalObject object) {
        if (null != object) {
            ObjectInfo objectInfo = object.getObjectInfo();
            if (null != objectInfo) {
                return isBottomDecoratorType(objectInfo.mType);
            }
        }
    ...
    /// cmpObject is trying to move to cmpSlot, return true if this object has
    /// sensitive about such moving.
    /// 1. check 'object' is a decorator and 'cmpObject' is not.
    /// 2. check 'object' is not a decorator and 'cmpObject is.
    /// 3. both are decorator or neither is decorator.
    public static boolean isObjectSlotConflict(NormalObject object, NormalObject cmpObject, ObjectSlot cmpSlot) {
        ObjectSlot hisSlot = cmpSlot;
        ObjectSlot mySlot = object.getObjectSlot();

        if (isBottomDecoratorType(object) && !isBottomDecoratorType(cmpObject)) {
            if (hisSlot.mSpanY == 1 || mySlot.isAbove(hisSlot) || hisSlot.isAbove(mySlot)) {
                return false;
            }
        } else if (isBottomDecoratorType(cmpObject) && !isBottomDecoratorType(object)){
            if (mySlot.mSpanY == 1 || mySlot.isAbove(hisSlot) || hisSlot.isAbove(mySlot)) {
                return false;
            }
        }

        boolean xConflict = !(hisSlot.isLeft(mySlot) || hisSlot.isRight(mySlot));
        boolean YConflict = !(hisSlot.isAbove(mySlot) || hisSlot.isBelow(mySlot));
//        boolean xConflict = !((hisSlot.mStartX <= mySlot.mStartX &&
//                (hisSlot.mStartX + hisSlot.mSpanX) <= mySlot.mStartX) ||
//                (hisSlot.mStartX >= (mySlot.mStartX + mySlot.mSpanX) &&
//                        (hisSlot.mStartX + hisSlot.mSpanX) >= (mySlot.mStartX + mySlot.mSpanX)));
//        boolean YConflict = !((hisSlot.mStartY <= mySlot.mStartY &&
//                (hisSlot.mStartY + hisSlot.mSpanY) <= mySlot.mStartY) ||
//                (hisSlot.mStartY >= (mySlot.mStartY + mySlot.mSpanY) &&
//                        (hisSlot.mStartY + hisSlot.mSpanY) >= (mySlot.mStartY + mySlot.mSpanY)));
    ...
    public static void createBottomDecorator(HomeScene homeScene, final SEObject parent, ObjectSlot slot) {
        ArrayList<ModelInfo> models = HomeManager.getInstance().getModelManager().findModelInfoByType(ObjectInfo.WALL_SHELF);
        if (null == models || models.isEmpty()) return;

//        ModelInfo modelInfo = HomeManager.getInstance().getModelManager().findModelInfo(objName);
        ModelInfo modelInfo = models.get(0);
        String objName = modelInfo.mName;

        final ObjectInfo objInfo = new ObjectInfo();
        objInfo.setModelInfo(modelInfo);
        if (ObjectInfo.WALL_SHELF.equals(objInfo.mType)) {
            SEScene scene = parent.getScene();
            objInfo.mObjectSlot.set(slot);
            // todo: why not set vesselID from existing slot in ObjectSlot.set(slot)?
            objInfo.mObjectSlot.mVesselID = slot.mVesselID;
//            objInfo.mObjectSlot.setVessel(slot);
            objInfo.mIndex = ProviderUtils.searchMaxIndex(scene, ProviderUtils.Tables.OBJECTS_INFO, objName) + 1;
            objInfo.mSceneName = scene.getSceneName();
            final NormalObject decorator = objInfo.CreateNormalObject(homeScene);
            HomeManager.getInstance().getModelManager().create(parent, decorator, new Runnable() {
                @Override
                public void run() {
                    parent.addChild(decorator, false);
                    decorator.setIsFresh(true);
                    decorator.initStatus();
                    decorator.setHasInit(true);
                    objInfo.saveToDB();
//            objInfo.saveToDB(new Runnable() {
//                public void run() {
////                    newDesk.updateAllChildSlotDB();
//                    // newDesk.changeLaptop();
//                }
//            });
                }
            });
    ...
    public static int calculateUsedSlotCount(SEObject object) {
        if (object instanceof NormalObject) {
            NormalObject normalObject = (NormalObject)object;
            if (!isBottomDecoratorType(normalObject)) {
                ObjectSlot slot = normalObject.getObjectSlot();
                return slot.mSpanX * slot.mSpanY;
            }
        }
