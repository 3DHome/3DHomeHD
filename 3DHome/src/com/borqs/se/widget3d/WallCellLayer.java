...
public class WallCellLayer extends VesselLayer {
    ...
    public boolean canHandleSlot(NormalObject object, float touchX, float touchY) {
        ...
        if (object.getObjectInfo().mSlotType == ObjectInfo.SLOT_TYPE_WALL
                || object.getObjectInfo().mSlotType == ObjectInfo.SLOT_TYPE_WALL) {
            return true;
        }
        return false;
    }
    ...
    public boolean onObjectMoveEvent(ACTION event, SEVector3f location) {
        ...
        switch (event) {
        case BEGIN:
            beginMoveOrScaleDecorator(getOnMoveObject());
        ...
        case FLY:
            ...
            if (mHasChangedSlot) {
                if (!onMoveOrScaleDecorator(getOnMoveObject(), location)) {
    ...
    private boolean canPlaceToWall(MOVE_DIRECTION moveDirection) {
        ...
        if (!canPlace) {
            ...
            SEVector2i nearestSlot = getNearestEmptySlotAtWall(info.mCellCountX, info.mCellCountY, moveSlot, existentSlot);
            clearMoveSlot();
    ...
    private boolean canPlaceToDirection(List<MoveObject> existentObjects, MoveObject moveObject,
        ...
        int wallSX = getHomeSceneInfo().mCellCountX;
        int wallSY = getHomeSceneInfo().mCellCountY;
    ...
    private List<MoveObject> calculateConflictObjects(List<MoveObject> existentObjs, List<ObjectSlot> cmpSlots) {
        List<MoveObject> conflictSlots = null;
        for (MoveObject normalObject : existentObjs) {
            ObjectSlot objectSlot = normalObject.getObjectInfo().mObjectSlot;
            for (ObjectSlot cmpSlot : cmpSlots) {
                boolean xConflict = !((cmpSlot.mStartX <= objectSlot.mStartX && (cmpSlot.mStartX + cmpSlot.mSpanX) <= objectSlot.mStartX) || (cmpSlot.mStartX >= (objectSlot.mStartX + objectSlot.mSpanX) && (cmpSlot.mStartX + cmpSlot.mSpanX) >= (objectSlot.mStartX + objectSlot.mSpanX)));
                boolean YConflict = !((cmpSlot.mStartY <= objectSlot.mStartY && (cmpSlot.mStartY + cmpSlot.mSpanY) <= objectSlot.mStartY) || (cmpSlot.mStartY >= (objectSlot.mStartY + objectSlot.mSpanY) && (cmpSlot.mStartY + cmpSlot.mSpanY) >= (objectSlot.mStartY + objectSlot.mSpanY)));
                if (xConflict && YConflict) {
                    if (conflictSlots == null) {
    ...
    private ObjectSlot calculateNearestSlot(SEVector3f location) {
        ...
        float wallZ = getHomeSceneInfo().mWallHeight / 2;
        float centerX = (getHomeSceneInfo().mWallPaddingLeft - getHomeSceneInfo().mWallPaddingRight) / 2;
        float centerZ = wallZ + (getHomeSceneInfo().mWallPaddingBottom - getHomeSceneInfo().mWallPaddingTop) / 2;
        ...
        float convertStartX = (start.getX() - centerX) / gridSizeX + getHomeSceneInfo().mCellCountX / 2f;
        float convertStartY = getHomeSceneInfo().mCellCountY / 2f - (start.getZ() - centerZ) / gridSizeY;

        slot.mStartX = Math.round(convertStartX);
        if (slot.mStartX < 0) {
            slot.mStartX = 0;
        } else if (slot.mStartX > getHomeSceneInfo().mCellCountX - slot.mSpanX) {
            slot.mStartX = getHomeSceneInfo().mCellCountX - slot.mSpanX;
        }
        slot.mStartY = Math.round(convertStartY);
        if (slot.mStartY < 0) {
            slot.mStartY = 0;
        } else if (slot.mStartY > getHomeSceneInfo().mCellCountY - slot.mSpanY) {
            slot.mStartY = getHomeSceneInfo().mCellCountY - slot.mSpanY;
        }
    ...
    public boolean placeObjectToVessel(NormalObject normalObject) {
        ...
        for (MoveObject moveObject : existentObjects) {
            if (ObjectInfo.isBottomDecoratorType(moveObject.mMoveObject)) {
                continue;
            }
    ...
    private void slotToWall(MOVE_DIRECTION moveDirection) {
        ...
        if (!calculateConflictObjects()) {
            ...
            if (canPlaceToWall(moveDirection)) {
                ...
                if (getOnMoveObject().getObjectSlot().mSlotIndex == mWallIndex) {
                    ...
                    HomeSceneInfo info = getHomeSceneInfo();
                    SEVector2i nearestSlot = getNearestEmptySlotAtWall(info.mCellCountX, info.mCellCountY, moveSlot, getExistentSlots());

                    if (nearestSlot != null) {
                        placeSlot = placeSlot.clone();
        ...
        if (placeSlot.mVesselID < 0) {
            NormalObject changeFrom = getOnMoveObject().getChangedToObj();
            if (changeFrom != null) {
                changeFrom.setVisible(true);
                changeFrom.setOnMoveStatus(false);
                if (!changeFrom.resetPostion()) {
                    changeFrom.getParent().removeChild(changeFrom, true);
                }
                changeFrom.setChangedToObj(null);
                getOnMoveObject().setChangedToObj(null);
            }
            ...
            final ObjectSlot targetSlot = placeSlot;
            playSlotAnimation(placeSlot, new SEAnimFinishListener() {
                public void onAnimationfinish() {
                    endMoveOrScaleDecorator(getOnMoveObject(), targetSlot);
                    handleSlotSuccess();
                }
            });
        }
    ...
    private SEVector2i getTheBestEmptySlotAtWall(int spanX, int spanY, List<ObjectSlot> existentSlot) {
        int sizeX = getHomeSceneInfo().mCellCountX;
        int sizeY = getHomeSceneInfo().mCellCountY;
...
//        return mConflictObjects.get(0);
    ...
    private void validateCurrentLayer(boolean b) {
        disableCurrentLayer();

        VesselLayer vesselLayer = getValidConflictVesselLayer();
        if (null != vesselLayer && vesselLayer != mCurrentLayer) {
...
//        return false;
    ...
    // decorator movement management begin
    private void beginMoveOrScaleDecorator(NormalObject movingObject) {
        if(ObjectInfo.isBottomDecoratorType(movingObject)) {
            saveCurrentShelfClickPosition(movingObject);
            if(mCurrentShelfPressPosition == CLICK_SHELF_MID) {
                getVesselObject().initObjectOnDecorator(movingObject);
            }
        } else {
            if (movingObject.isAcceptedDecoratorSlot()) {
                getVesselObject().initObjectOnDecorator(movingObject);
            }
        }
    }
    private boolean endMoveOrScaleDecorator(NormalObject movingObject, ObjectSlot placeSlot) {
        if (ObjectInfo.isBottomDecoratorType(movingObject)) {
            if(mCurrentShelfPressPosition == CLICK_SHELF_MID) {
                return getVesselObject().finishObjectOnDecorator(movingObject, placeSlot);
            }
        } else {
            if (movingObject.isAcceptedDecoratorSlot()) {
                return getVesselObject().finishObjectOnDecorator(movingObject, placeSlot);
    ...
    // invocation via move, up, fly, finish
    private boolean onMoveOrScaleDecorator(NormalObject movingObject, SEVector3f location) {
//        return false;
        if(ObjectInfo.isBottomDecoratorType(movingObject)) {
            if(mCurrentShelfPressPosition == CLICK_SHELF_LEFT ||
                    mCurrentShelfPressPosition == CLICK_SHELF_RIGHT) {
                changeShelfScale(movingObject);
            } else {
                updateShelfPosition(location, movingObject);
            }
            return true;
        }
        return false;
    ...
    private void updateShelfPosition(SEVector3f location, NormalObject movingObject) {

    }

    private void changeShelfScale(NormalObject movingObject) {
        // todo: scale calculation
    }

    private boolean slotDecoratorToWall(NormalObject movingObject, SEVector3f location, MOVE_DIRECTION direction) {
        return false;
    }

    protected boolean isMovingObject(NormalObject movingObject) {
        return !(ObjectInfo.isBottomDecoratorType(movingObject)) ||
                mCurrentShelfPressPosition == CLICK_SHELF_MID;
    }

    private int mCurrentShelfPressPosition;
    private static final int CLICK_SHELF_LEFT = 0;
    private static final int CLICK_SHELF_RIGHT = 1;
    private static final int CLICK_SHELF_MID = 2;

    private void saveCurrentShelfClickPosition(NormalObject movingObject) {
//        SEVector3f realSpan = movingObject.getObjectSize();
        final float width = movingObject.getWallCellWidth(movingObject.getObjectSlot().mSpanX);
        SEVector3f center = movingObject.getUserTranslate();
        final float originLeft = center.getX() - 0.5f * width;
        final float originRight = center.getX() + 0.5f * width;
        int currentClickPosition = calculateShelfClickPosition(originLeft, originRight,
                mMoveLocation.mD[0], width);
        mCurrentShelfPressPosition = currentClickPosition;
    }

    private static int calculateShelfClickPosition(float left, float right, float currentPosition, float shelfWidth) {
    ...
    // decorator movement management end
    ...
    /// existing object management end
