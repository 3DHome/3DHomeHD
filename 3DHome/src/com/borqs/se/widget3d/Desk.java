...
public class Desk extends VesselObject {
    ...
    public void initStatus() {
        ...
        if (!TextUtils.isEmpty(observerCameraData)) {
            ...
            mObserverCameraData.mFov = Float.parseFloat(data[0]);
            mObserverCameraData.mLocation = new SEVector3f(Float.parseFloat(data[1]), Float.parseFloat(data[2]),
                    Float.parseFloat(data[3]));
            mObserverCameraData.mAxisZ = new SEVector3f(Float.parseFloat(data[4]), Float.parseFloat(data[5]),
        ...
        getUserTransParas().reset();
        setUserTransParas();
        setVisible(true);
        setHasInit(true);
    }

    public void changeDeskTo(String newDeskName) {
        String objName = newDeskName;
        if (mName.equals(objName)) {
            return;
        }
        ObjectInfo objInfo = new ObjectInfo();
        objInfo.setModelInfo(HomeManager.getInstance().getModelManager().findModelInfo(objName));
        if ("Desk".equals(objInfo.mType)) {
            ...
            final Desk newDesk = (Desk) objInfo.CreateNormalObject(getHomeScene());
            HomeManager.getInstance().getModelManager().createQuickly(getParent(), newDesk);
            getParent().addChild(newDesk, false);
            newDesk.initStatus();
            newDesk.setHasInit(true);
            moveAllChildToVesselObject(newDesk);
            getParent().removeChild(this, true);
            objInfo.mObjectSlot.mVesselID = getObjectSlot().mVesselID;
