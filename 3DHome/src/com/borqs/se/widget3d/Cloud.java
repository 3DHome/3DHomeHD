...
public class Cloud extends NormalObject implements CameraChangedListener {
    ...
    public void initStatus() {
        ...
        if (!TextUtils.isEmpty(observerCameraData)) {
            ...
            mObserverCameraData_Land.mFov = Float.parseFloat(data[0]);
            mObserverCameraData_Land.mLocation = new SEVector3f(Float.parseFloat(data[1]), Float.parseFloat(data[2]),
                    Float.parseFloat(data[3]));
            mObserverCameraData_Land.mAxisZ = new SEVector3f(Float.parseFloat(data[4]), Float.parseFloat(data[5]),
                    Float.parseFloat(data[6]));

        }
        getHomeScene().setSky(this);
        mSkyImgKey = "assets/base/sky/qingtianbogyun.jpg";
        float skyHeight = getHomeSceneInfo().mWallHeight + 100;
    ...
    public SECameraData getObserveCamera() {
        if (HomeManager.getInstance().getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            if (mObserverCameraData_Land != null) {
        ...
        float h = getHomeSceneInfo().mWallHeight + 50;
        float r = getHomeSceneInfo().mSkyRadius;
        float paras = (float) (0.6f * r / Math.tan(SIGHT_FOV * Math.PI / 360));
        SEVector3f screenOrientation = new SEVector3f(0, (float) Math.cos(MAX_SIGHT_ANGLE),
                (float) Math.sin(MAX_SIGHT_ANGLE));
        SEVector3f centerLocation = new SEVector3f(0, 0, h);
        ...
        SEVector3f top = new SEVector3f(0, -0.85f * r, h);
    ...
    protected void preCheckWeatherSituation() {
    ...
    protected void hide(final SEAnimFinishListener l) {
        if (mOnShow) {
            mOnShow = false;
            stopAllAnimation(null);
    ...
    protected void processWeather_changed(int type, boolean isNight) {
    ...
    private void processWeather_CreateParticle(int type, boolean isNight) {
        float houseW = getHomeSceneInfo().mWallRadius * 2;
        float houseH = getHomeSceneInfo().mWallHeight;
        switch (type) {
        case WeatherConditions.CONDITION_TYPE_SNOW:
            SELoadResThread.getInstance().process(new Runnable() {
                public void run() {
    ...
    private void processWeather_LoadSkyObject(int type, boolean isNight) {
        ...
        case WeatherConditions.CONDITION_TYPE_SUN:
            if (isNight) {
                int lunarDay = HomeManager.getInstance().getLunarDay();
        ...
        case WeatherConditions.CONDITION_TYPE_FOG:
            if (isNight) {
                ...
                if (lunarDay > 1 && lunarDay < 30) {
                    ...
                    getScene().getContentObject().addChild(mMoon, true);
                    mSkyObjects.add(mMoon);
                    mMoon.setUseUserColor(0.5f, 0.5f, 0.5f);
                }
            }
            break;
        default:
    ...
    protected void processWeather_ShowWeatherImage() {
    }
    ...
    private void initSkyFacePara() {
        ...
        mSkyHeight = getHomeSceneInfo().mWallHeight + 100;
        mWallRadius = getHomeSceneInfo().mWallRadius + 100;
    ...
    public static Cloud instance(HomeScene scene, String name, int index) {
        return new WeatherCloud(scene, name, index);
    }

    public static class WeatherCloud extends Cloud implements WeatherBindedCallBack {
        private IWeatherService mService;
        private long mRequestWeatherTime;

        public WeatherCloud(HomeScene scene, String name, int index) {
            super(scene, name, index);
        ...
        private void weatherUpdate(final boolean forceUpdate) {
            if (mService == null) {
                return;
            }
        ...
        protected void preCheckWeatherSituation() {
            if (mService == null) {
                super.preCheckWeatherSituation();
            } else {
                try {
