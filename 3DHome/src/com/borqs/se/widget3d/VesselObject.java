...
public abstract class VesselObject extends NormalObject implements AppCallBack {
    ...
    /// move all children to new vessel object begin
    /// the method comes from Desk class
    protected void moveAllChildToVesselObject(VesselObject newVessel) {
        int index = 0;
        List<SEObject> childObjects = new ArrayList<SEObject>();
        childObjects.addAll(getChildObjects());
        Collections.sort(childObjects, new SortByIndex());
        for (SEObject child : childObjects) {
            if (child instanceof NormalObject) {
                NormalObject normalObject = (NormalObject) child;
                normalObject.getObjectSlot().mSlotIndex = index;
                if (index < newVessel.getVesselCapability()) {
                    normalObject.changeParent(newVessel);
                    normalObject.initStatus();
                    normalObject.setHasInit(true);
                }
                index++;
            }
        }
    ...
    protected void appendAllChildrenToVesselObject(VesselObject newVessel) {
        int index = newVessel.getVesselCapability() - getVesselCapability();
        List<SEObject> childObjects = new ArrayList<SEObject>();
        childObjects.addAll(getChildObjects());
        Collections.sort(childObjects, new SortByIndex());

        int offsetSlotIndex = newVessel.getObjectSlot().mSpanX - 1;
        for (SEObject child : childObjects) {
            if (child instanceof NormalObject) {
                NormalObject normalObject = (NormalObject) child;
                normalObject.getObjectSlot().mSlotIndex = index;
                normalObject.getObjectSlot().mStartX += offsetSlotIndex;
                if (index < newVessel.getVesselCapability()) {
                    normalObject.changeParent(newVessel);
                    normalObject.initStatus();
                    normalObject.setHasInit(true);
    ...
    /// move all children to new vessel object end

    protected final void onLabelShown(int index) {
        ArrayList<SEObject> children = new ArrayList<SEObject>();
        children.addAll(getChildObjects());
        for (SEObject child : children) {
            ensureDecoratorForChild(child, index);
            if (child instanceof VesselObject) {
                ((VesselObject)child).onLabelShown(index);
                ((NormalObject)child).onLabelShown(index == 0);
            } else if (child instanceof NormalObject) {
                ((NormalObject)child).onLabelShown(index == 0);
            }
        }
    }
