...
public class WidgetResizeLayer implements ResizeLayerInterface {
    ...
    public void startResize() {
        ...
        mFinallyResizeRect.set(objectSlot.left(), objectSlot.top(),
                objectSlot.right(), objectSlot.bottom());
    ...
    public boolean handleResizeTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            case RIGHT_TOP:
                ...
                } else if (newRight > mScene.getHomeSceneInfo().mCellCountX) {
                    newRight = mScene.getHomeSceneInfo().mCellCountX;
                }
                if (newBottom - newTop < mMinY - 0.2f) {
                    newTop = newBottom - mMinY + 0.2f;
                } else if (newTop < 0) {
            ...
            case LEFT_BOTTOM:
                ...
                } else if (newBottom > mScene.getHomeSceneInfo().mCellCountY) {
                    newBottom = mScene.getHomeSceneInfo().mCellCountY;
                }
                break;
            case RIGHT_BOTTOM:
                newRight = newRight + (touchX - mPreTouchX) * mTouchParaX;
    ...
    private float getWidthOfWidget(float spanX) {
        float gridSizeX = mScene.getHomeSceneInfo().mCellWidth + mScene.getHomeSceneInfo().mWidthGap;
        return spanX * gridSizeX - mScene.getHomeSceneInfo().mWidthGap;
    }

    private float getHeightOfWidget(float spanY) {
        float gridSizeY = mScene.getHomeSceneInfo().mCellHeight + mScene.getHomeSceneInfo().mHeightGap;
        return spanY * gridSizeY - mScene.getHomeSceneInfo().mHeightGap;
    }

    private SEVector3f getResizeRectPostion(RectF rect) {
        float gridSizeX = mScene.getHomeSceneInfo().mCellWidth + mScene.getHomeSceneInfo().mWidthGap;
        float gridSizeY = mScene.getHomeSceneInfo().mCellHeight + mScene.getHomeSceneInfo().mHeightGap;
        float offsetX = (rect.left + rect.width() / 2.f) * gridSizeX - mScene.getHomeSceneInfo().mCellCountX
                * gridSizeX / 2.f
                + (mScene.getHomeSceneInfo().mWallPaddingLeft - mScene.getHomeSceneInfo().mWallPaddingRight) / 2;
        float offsetZ = mScene.getHomeSceneInfo().mCellCountY * gridSizeY / 2.f - (rect.top + rect.height() / 2.f)
                * gridSizeY
                + (mScene.getHomeSceneInfo().mWallPaddingBottom - mScene.getHomeSceneInfo().mWallPaddingTop) / 2;
        return new SEVector3f(offsetX, -20, offsetZ);
    }
    ...
    private Rect getMaxCanSlotRect(Rect from, Rect to, START_POSITION startPostion) {
        int sizeX = mScene.getHomeSceneInfo().mCellCountX;
        int sizeY = mScene.getHomeSceneInfo().mCellCountY;
