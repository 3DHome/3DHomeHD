...
public class DragLayer {
    ...
    public void startDrag(NormalObject onDragObject, float touchX, float touchY) {
        if (onDragObject != null) {
            ...
            if ("App".equals(mOnDragObject.getObjectInfo().mType)
                    || "Shortcut".equals(mOnDragObject.getObjectInfo().mType)
                    || "IconBox".equals(mOnDragObject.getObjectInfo().mType)) {
    ...
    private void loadAndShowRecycle(VesselLayer currentLayer) {
        ...
        float y = mHomeScene.getHomeSceneInfo().mWallRadius * scale;
        ...
        boundOfRecycle.left = (int) (mCamera.getWidth() / 2 - 25 * mHomeScene.getScreenDensity());
        boundOfRecycle.right = (int) (mCamera.getWidth() / 2 + 25 * mHomeScene.getScreenDensity());
    ...
    private void loadAndShowRecycle(SEVector3f srcTranslate, SEVector3f desTranslate, float scale) {
        ...
        HomeManager.getInstance().getModelManager().createQuickly(mHomeScene.getContentObject(), mRecycle);
