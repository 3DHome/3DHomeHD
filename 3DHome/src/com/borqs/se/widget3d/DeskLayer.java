...
public class DeskLayer extends VesselLayer {
    ...
    public boolean canHandleSlot(NormalObject object, float touchX, float touchY) {
        ...
        if (object instanceof AppObject && touchX > 0) {
            float lineGround = mCamera.worldToScreenCoordinate(new SEVector3f(0, getHomeSceneInfo().mWallRadius, 0))
                    .getY();
    ...
    public boolean setOnLayerModel(NormalObject onMoveObject, boolean onLayerModel) {
        ...
        if (onLayerModel) {
            if (onMoveObject instanceof AppObject) {
                AppObject appObject = (AppObject) onMoveObject;
                appObject.setPressed(false);
                IconBox iconBox = appObject.changeToIconBox();
                setOnMoveObject(iconBox);
                getHomeScene().getDragLayer().changeOnDragObject(iconBox);
            }
            setInRecycle(false);
    ...
    private SEVector3f getTouchLocation(SERay ray, float z) {
        ...
        if (distance > getHomeSceneInfo().mSkyRadius * 0.8f) {
            distance = getHomeSceneInfo().mSkyRadius * 0.8f;
            if (touchLocZ.getY() > 0) {
