...
public class Flyer extends NormalObject {   
    ...
    private void onStartAnimation() {
        setNeedFog(HomeManager.getInstance().isFoggyWeather());
    ...
    private void playAnimation(float speed, boolean reversed, SEAnimFinishListener l) {
        ...
        float y;
        int animCount;
        if (HomeManager.getInstance().getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
            skyHeight = getHomeSceneInfo().mWallHeight + 260;
            y = (float) (Math.random() * 100) + getHomeSceneInfo().mWallRadius * 0.15f;
            animCount = (int) (2000 / speed);
        } else {
            skyHeight = getHomeSceneInfo().mWallHeight + 480;
            y = (float) (Math.random() * 200) + getHomeSceneInfo().mWallRadius * 0.3f;
    ...
    private void initObject() {
        ...
        float wallWidth = getHomeSceneInfo().mWallWidth;
        if (HomeManager.getInstance().getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
        ...
        String bannerImageName = mBanner.getImageName();
        ImageItem imageItem = getObjectInfo().mModelInfo.mImageInfo.getImageItem(bannerImageName);
        mBannerImageKey = imageItem.mNewPath;
