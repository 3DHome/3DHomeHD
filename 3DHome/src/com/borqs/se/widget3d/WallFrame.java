...
public class WallFrame extends WallObject {
    ...
    public void initStatus() {
        ...
        mSaveImagePath = "/data/data/com.borqs.se/files/" + getScene().getSceneName() + mName + mIndex + ".png";
            ...
            SELoadResThread.getInstance().process(new Runnable() {
                public void run() {
                    ...
                    if (imageFileUri != null) {
                        ...
                        bm = HomeUtils.decodeSampledBitmapFromResource(HomeUtils.TMPDATA_IMAGE_PATH,
                            Max_IMAGE_SIZE, Max_IMAGE_SIZE);
                    }
                    if (bm == null) {
                        return;
                    }
                    int imageSize = bm.getWidth() > bm.getHeight() ? SEUtils.higherPower2(bm.getWidth())
                            : SEUtils.higherPower2(bm.getHeight());
                    if (imageSize > Max_IMAGE_SIZE) {
                        imageSize = Max_IMAGE_SIZE;
                    }
                    if (bm.getWidth() > bm.getHeight()) {
                        bm = Bitmap.createScaledBitmap(bm, imageSize, imageSize * bm.getHeight() / bm.getWidth(), true);
                    } else {
                        bm = Bitmap.createScaledBitmap(bm, imageSize * bm.getWidth() / bm.getHeight(), imageSize, true);
                    }

                    Bitmap des = Bitmap.createBitmap(imageSize, imageSize, Bitmap.Config.RGB_565);
                    int left;
                    int top;
                    if (bm.getWidth() > bm.getHeight()) {
                        left = 0;
                        top = (imageSize - bm.getHeight()) / 2;
                    } else {
                        left = (imageSize - bm.getWidth()) / 2;
                        top = 0;
                    }
    ...
    private float getWidthOfWidget(float spanX) {
        return getWallCellWidth(spanX);
    }

    private float getHeightOfWidget(float spanY) {
        return getWallCellHeight(spanY);
    }
