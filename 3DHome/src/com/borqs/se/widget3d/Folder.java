        ...
        new SECommand(getScene()) {
            public void run() {
                if (!hasBeenReleased()) {
                    ...
                    String label = isLabelShown() ? getObjectInfo().mDisplayName : null;
                    Bitmap iconWithText = AppObject.getAppIconBitmap(getHomeScene(), icon, null,
                            label, false);
    ...
    private void showExpand() {
        if (!mOnDialogShow && !mOnExpandFolderUpdate) {
            ...
            if (HomeManager.getInstance().getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
                ...
                // 获取墙面中心
                SEVector3f centerOfWall = new SEVector3f(0, getHomeSceneInfo().mWallRadius,
                        getHomeSceneInfo().mWallHeight / 2);
                // 获取相机位置到墙面中心的向量
...
//        }
    ...
    private class FolderExpandDialog extends VesselObject {
        ...
        public SETransParas getTransParasInVessel(NormalObject needPlaceObj, ObjectSlot objectSlot) {
            ...
            float scale = mGridSizeX / getHomeSceneInfo().mCellWidth * 0.70f;
