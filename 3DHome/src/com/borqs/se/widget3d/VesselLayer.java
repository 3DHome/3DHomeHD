...
public class VesselLayer {
    ...
    private List<MoveObject> mExistentObjects;
    ...
    public boolean setOnLayerModel(NormalObject onMoveObject, boolean onLayerModel) {
        ...
        mVesselObject.onFocusChanged(onLayerModel);
        return true;
    }
    ...
    protected boolean placeObjectToVesselDirectly(NormalObject normalObject) {
        if (normalObject.getObjectSlot().mVesselID == getVesselId()) {
            ...
            mVesselObject.notifyDecoratorSlotDown(normalObject);
            return true;
        }
        return false;
    }
    ...
    public void setOnMoveObject(NormalObject normalObject) {
        mOnMoveObject = new MoveObject();
        mOnMoveObject.mMoveObject = normalObject;
        mOnMoveObject.mMoveSlot = normalObject.getObjectSlot().clone();
    }
    ...
    // / implement the method in sub class
    ...
    public static class MoveObject {
        ...
        private void performNeverStopAnimation(final SEVector3f moveDirect) {
            ...
            mShakeAnimation = new SENeverStopAnimation(mMoveObject.getHomeScene(), 40) {
                ...
                public void onAnimationRun(float value) {
                    ...
                    SEVector3f location = moveDirect.mul(value * mMoveObject.getHomeSceneInfo().mCellWidth * 0.036f);
        ...
        public void performConflictAnimation(VesselObject vessel, boolean end) {
            ...
            if (mMoveSlot == null) {
                ...
                SEVector3f desLocation = object.getSlotTranslateInVessel(vessel, getObjectSlot());
            ...
            SEVector3f desLocation = object.getSlotTranslateInVessel(vessel, mMoveSlot); //vessel.getTransParasInVessel(object, mMoveSlot).mTranslate;
            SEVector3f fromLocation = object.getSlotTranslateInVessel(vessel, getObjectSlot()); //vessel.getTransParasInVessel(object, getObjectSlot()).mTranslate;
            SEVector3f srcLocation = object.getUserTranslate();
        ...
        protected List<MoveObject> calculateConflictObjects(List<MoveObject> existentObjs, ObjectSlot cmpSlot) {
            ...
            for (MoveObject normalObject : existentObjs) {
                if (ObjectInfo.isObjectSlotConflict(normalObject.mMoveObject, mMoveObject, cmpSlot)) {
        ...
        protected List<MoveObject> calculateConflictObjects(List<MoveObject> existentObjs,
                                                            List<MoveObject> movingObjects,
                                                            int moveX, int moveY) {
            List<MoveObject> conflictSlots = null;
            for (MoveObject normalObject : existentObjs) {
                for (MoveObject cmpObject : movingObjects) {
                    if (normalObject.mMoveObject.isObjectSlotConflict(cmpObject.mMoveObject, moveX, moveY)) {
                        if (conflictSlots == null) {
                            conflictSlots = new ArrayList<MoveObject>();
                        }
                        conflictSlots.add(normalObject);
        ...
        protected boolean fetchConflictObjects(List<MoveObject> objectList) {
            mConflictObjects = calculateConflictObjects(objectList, mMoveSlot);
        ...
        // conflict object list end
    }

    /// conflict task end

    /// util method move from WallCellLayer to calculate a target slot (x, y) that can place
    /// the bounder of moveSlot, which should not overlap any of the existing slot.
    protected static SEVector.SEVector2i getNearestEmptySlotAtWall(int sizeX, int sizeY, List<ObjectSlot> moveSlot, List<ObjectSlot> existentSlot) {
//        int sizeX = homeSceneInfo.mCellCountX;
//        int sizeY = homeSceneInfo.mCellCountY;
