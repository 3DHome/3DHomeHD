...
public class ApplicationMenu extends SEObject implements AppCallBack {
    ...
    private int mNewSceneWidth;
    private int mNewSceneHeight;

    @Override
    public void notifySurfaceChanged(int width, int height) {
        super.notifySurfaceChanged(width, height);
        mNewSceneWidth = width;
        mNewSceneHeight = height;
        executeDelayed(2000);
    }

    private Runnable mExecuteTask = new Runnable() {
        public void run() {
            new SECommand(getScene(), mTaskID) {
                public void run() {
                    if (hasBeenAddedToEngine() && (mScreenW != mNewSceneWidth || mScreenH != mNewSceneHeight)) {
                        mScreenW = mNewSceneWidth;
                        mScreenH = mNewSceneHeight;
                        caculateAppsMenuWallSize(mScreenW, mScreenH);

                        if (mRotateMenu != null) {
                            mRotateMenu.getParent().removeChild(mRotateMenu, true);
                            mRotateMenu = new SEObject(getScene(), "RotateMenu");
                            mRotateMenu.setIsGroup(true);
                            addChild(mRotateMenu, true);
                            mRotateMenu.setPressType(PRESS_TYPE.NONE);
                        ...
                        if (mBackground != null) {
                            mBackground.getParent().removeChild(mBackground, true);
                            mBackground = new SEObject(getScene(), "application_background");
                            SERect3D rect = new SERect3D(mAppsMenuWallWidth, mAppsMenuWallHeight);
                            SEObjectFactory.createRectangle(mBackground, rect, IMAGE_TYPE.COLOR, null, null, null,
                                    new float[] { 0, 0, 0 }, true);
                        ...
                        if (mHomeScene.getStatus(HomeScene.STATUS_APP_MENU)) {
                            ...
                            float y = (float) (mAppsMenuWallWidth
                                    / (2 * Math.tan(getCamera().getFov() * Math.PI / 360)) + mFaceRadius)
                                    * SCALE;
                            mRotateMenu.getUserTransParas().mTranslate = new SEVector3f(0, y, 0).add(getCamera()
                                    .getLocation());
    ...
    private int mTaskID = 100000000;

    public void executeDelayed(long delay) {
        getScene().removeMessage(mTaskID);
...
//                    if (mDesk != null) {
//                        mDesk.hide(new SEAnimFinishListener() {
//                            public void onAnimationfinish() {
//                            }
//                        });
//                    }
    ...
    private void setCubeToRightPosition() {
        ...
        if (getScene() instanceof HomeScene) {
            ...
            mBackground.getUserTransParas().set(getBackgroundTransparas());
            mBackground.setUserTransParas();
            SEEmptyAnimation updateSceneAnimation = new SEEmptyAnimation(getScene(), 0, 1, 10) {
                @Override
            ...
            updateSceneAnimation.setAnimFinishListener(new SEAnimFinishListener() {
                public void onAnimationfinish() {
                    ...
                    SETranslateAnimation moveDockAnimation = new SETranslateAnimation(getScene(), mDocMenuGroup,
                            fromLocation, toLocation, 4);
                    moveDockAnimation.execute();
                    moveDockAnimation.setAnimFinishListener(new SEAnimFinishListener() {
                        public void onAnimationfinish() {
                            mDisableTouch = false;
                            mPreViewCylinder = new PreViewCylinder(getScene(), "PreViewCylinder");
                            mRotateMenu.addChild(mPreViewCylinder, true);
...
//                if (!mNeedSetBindObject.getObjectInfo().isShortcut()
//                        && mNeedSetBindObject.getObjectInfo().mComponentName == null) {
//                    mNeedSetBindObject.getObjectInfo().releaseDB();
//                    mNeedSetBindObject.getParent().removeChild(mNeedSetBindObject, true);
//
//                }
...
//                    if (mDesk != null) {
//                        mDesk.show(new SEAnimFinishListener() {
//                            public void onAnimationfinish() {
//                                if (l != null) {
//                                    l.onAnimationfinish();
//                                }
...
//                    }
    ...
    private void forceReload(List<ItemInfo> apps) {
    ...
    private class DocMenuGroup extends SEObject {
        ...
        public void onRenderFinish(SECamera camera) {
            if (mCurrentShowType == TYPE_BIND_APP) {
                ...
                mMenuC = new SEImageView(getScene(), "DocMenu_C");
                mMenuC.setBackground(R.drawable.dock_sort_normal);
                mMenuC.setSize(dockSize, dockSize);
                addChild(mMenuC, true);
                mMenuC.setUserTranslate(new SEVector3f(screenW * 3 / 8, -10, 0));
                mMenuC.setOnClickListener(new OnTouchListener() {
                    @Override
                    public void run(SEObject obj) {
                        showSortTypes();
                    }
