...
public class SettingsActivity extends PreferenceActivity implements OnPreferenceClickListener,
    ...
    protected void onResume() {
        ...
        mPrefShelf.setChecked(getShowShelfSetting(this));
    }
    ...
    private void initSetting() {
        ...
        initWallpaperSizePreference();

        initDecoratorShelf();
        initDecoratorPreference();
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference.getKey().equals(KEY_FPS)) {
            ...
            setShowShelfSetting(objValue);
//        } else if (KEY_WALL_LABEL.equals(preference.getKey())) {
//            boolean showLabel = (Boolean)objValue;
//            if (showLabel != getItemLabelShowOnWall(this)) {
//                setItemLabelShowOnWall(this, showLabel);
//                HomeManager.notifyWallLabelShow(showLabel);
//            }
        } else if (PREF_KEY_ICON_DECORATOR.equals(preference.getKey())) {
            int value = Integer.parseInt((String) objValue);
            int current = getPreferredDecorator(this, 0);
            if (value != current) {
                HomeManager.notifyWallLabelShow(value);
                setPreferredDecorator(this, value);
            }
        } else if (PREF_KEY_PAPER_SIZE.equals(preference.getKey())) {
            int size = Integer.parseInt((String) objValue);
            setPreferredWallpaperSize(this, size);
    ...
    // show wall shelf decorator begin.
    private static final String KEY_SHOW_SHELF = "app_icon_decorator_customization";
    private CheckBoxPreference mPrefShelf;
    private void initDecoratorShelf() {
        boolean showFlag = getShowShelfSetting(this);
        checkToEnableShowShelfSetting(showFlag);
        mPrefShelf = (CheckBoxPreference) findPreference(KEY_SHOW_SHELF);
        mPrefShelf.setOnPreferenceChangeListener(this);
//        if (!HomeUtils.DEBUG) {
//            mDisplay.removePreference(mPrefShelf);
//        }
    ...
    public static boolean getShowShelfSetting(Context context) {
        SharedPreferences settings = context.getSharedPreferences(PREFS_SETTING_NAME, 0);
        return settings.getBoolean(KEY_SHOW_SHELF, false);

    }

    private void setShowShelfSetting(Object objValue) {
        boolean state = (Boolean) objValue;
        if (state == getShowShelfSetting(this)) {
            return;
        }

        if (state && !checkToEnableShowShelfSetting(state)) {
            // disable the option while shelf is not ready
//            Toast.makeText(HomeManager.getInstance().getContext(),
//                    R.string.wall_shelf_missing_prompt, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        int decorator = state ? getPreferredDecorator(this, 0) : 0;
        HomeManager.notifyWallLabelShow(decorator);
    }

    private boolean checkToEnableShowShelfSetting(boolean download) {
        return HomeUtils.checkAndDownloadDecorator(download);
    }
    // show wall shelf decorator end.
    private static boolean getBooleanSettings(Context context, String key, boolean defValue) {
        SharedPreferences settings = context.getSharedPreferences(SettingsActivity.PREFS_SETTING_NAME, 0);
        return settings.getBoolean(key, defValue);
    }


    private static void setBooleanSettings(Context context, String key, Boolean value) {
        SharedPreferences settings = context.getSharedPreferences(SettingsActivity.PREFS_SETTING_NAME, 0);
        SharedPreferences.Editor editor = settings.edit();
    ...
    private static final String PREF_KEY_ICON_DECORATOR = "AppDecoratorSettings";
    private void setPreferredDecorator(Context context, int value) {
        setIntSettings(context, PREF_KEY_ICON_DECORATOR, value);
        updateDecoratorDisplay();
    }

    public static int getPreferredDecorator(Context context, int defVal) {
        return getIntSettings(context, PREF_KEY_ICON_DECORATOR, defVal);
    }

    private ListPreference mPaperSizePreference;
    private void initWallpaperSizePreference() {
        mPaperSizePreference = (ListPreference)findPreference(PREF_KEY_PAPER_SIZE);
        if (null != mPaperSizePreference) {
            mPaperSizePreference.setOnPreferenceChangeListener(this);
            mPaperSizePreference.setValue(String.valueOf(HomeManager.getWallpaperMaxSize()));
            updateWallpaperDisplay();
        }
    }
    private void updateWallpaperDisplay() {
        int index = 0;
        int value = HomeManager.getWallpaperMaxSize();
        String[] values = getResources().getStringArray(R.array.wallpaper_size_values);
        String[] entries = getResources().getStringArray(R.array.wallpaper_size_entries);
        String intervalString = String.valueOf(value);
        ...
        if (index < entries.length) {
            mPaperSizePreference.setSummary(entries[index]);
        }
    }

    private ListPreference mDecoratorPreference;
    private void initDecoratorPreference() {
        mDecoratorPreference = (ListPreference)findPreference(PREF_KEY_ICON_DECORATOR);
        if (null != mDecoratorPreference) {
            mDecoratorPreference.setOnPreferenceChangeListener(this);
            mDecoratorPreference.setValue(String.valueOf(getPreferredDecorator(this, 0)));
            updateDecoratorDisplay();
        }
    }
    private void updateDecoratorDisplay() {
        int index = -1;
        int current = getPreferredDecorator(this, 0);
        String[] values = getResources().getStringArray(R.array.app_icon_decorator_values);
        String[] entries = getResources().getStringArray(R.array.app_icon_decorator_entries);
        String currentStr = String.valueOf(current);
        for (int i = 0; i < values.length; i++) {
            if (currentStr.equals(values[i])) {
                index = i;
                break;
            }
        }
        ...
        if (index >= 0 && index < entries.length) {
            mDecoratorPreference.setSummary(entries[index]);
        }
    }

    public static int getItemLabelShowOnWall(Context context) {
        if (getShowShelfSetting(context)) {
            return getPreferredDecorator(context, 0);
        } else {
            return 0;
        }
    }
