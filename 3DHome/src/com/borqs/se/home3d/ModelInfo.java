...
public class ModelInfo {
    ...
    public static ModelInfo CreateFromXml(XmlPullParser parser, String id, String localPath) throws XmlPullParserException,
            IOException {
        ...
        String supprotScreen = parser.getAttributeValue(null, ModelColumns.SUPPORT_SCREEN);
        if (!TextUtils.isEmpty(supprotScreen)) {
            if ("portrait".equals(supprotScreen)) {
                info.mSupportScreen = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;
            } else if ("landscape".equals(supprotScreen)) {
                info.mSupportScreen = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE;
            }
        }
    ...
    public static ModelInfo CreateFromDB(Cursor cursor) {
        ...
        info.mSupportScreen = cursor.getInt(cursor.getColumnIndexOrThrow(ModelColumns.SUPPORT_SCREEN));
        ...
        if (componentCursor != null) {
            while (componentCursor.moveToNext()) {
                ...
                //多个灯源在某些手机上有BUG原因不详，未解决前只允许使用一种灯光
//                if (componentAttribute.mLightNames != null) {
//                    String[] lights = componentAttribute.mLightNames.split(ProviderUtils.SPLIT_SYMBOL);
//                    if (lights.length > 1) {
//                        if ("help".equals(lights[1])) {
//                            componentAttribute.mLightNames = "help";
//                        } else {
//                            componentAttribute.mLightNames = "mainsun";
//                        }
//                    } 
//                }
    ...
    public void updateFromNewModelInfo(ModelInfo source) {
        mID = source.mID;
        mName = source.mName;
        mType = source.mType;
        mSceneFile = source.mSceneFile;
        mBasedataFile = source.mBasedataFile;
        mAssetsPath = source.mAssetsPath;
        mSlotType = source.mSlotType;
        mSpanX = source.mSpanX;
        mSpanY = source.mSpanY;
        mCompensationTrans = source.mCompensationTrans;
        mKeyWords = source.mKeyWords;
        mComponentName = source.mComponentName;
        mImageInfo = source.mImageInfo;
        mComponentAttributes = source.mComponentAttributes;
        mProductId = source.mProductId;
        mIsDownloaded = source.mIsDownloaded;
        mActionStatus = source.mActionStatus;
    }
    ...
    public void updateImageKey(final String imageName, final String newKey) {
        UpdateDBThread.getInstance().process(new Runnable() {
            public void run() {
                updateImageKeyInForeground(imageName, newKey);
            }
        });
    }

    public void updateImageKeyInForeground(String imageName, String newKey) {
        final ImageItem imageItem = mImageInfo.getImageItem(imageName);
        if (imageItem != null) {
            imageItem.mNewPath = TextUtils.isEmpty(newKey) ? getImageOldKey(imageName) : newKey;
            ContentValues values = new ContentValues();
            values.put(ModelColumns.IMAGE_NEW_PATH, imageItem.mNewPath);
            String where = getImageQueryByName(imageItem.mImageName);
    ...
    // heavy task that should be invoked within thread from caller
//    public synchronized void updateImageBundle(ArrayList<ImageItem> list) {
//        Set<String> keySet = paperBundle.keySet();
//        ImageItem imageItem;
//        String newKey;
//        for (String imageName : keySet) {
//            imageItem = mImageInfo.getImageItem(imageName);
//            if (null == imageItem) {
//                continue;
//            }
//            newKey = paperBundle.get(imageName);
//            if (TextUtils.isEmpty(newKey)) {
//                newKey = imageItem.mPath; // reset to default path.
//                if (TextUtils.isEmpty(newKey)) {
//                    continue;
//                }
//            }
//
//            imageItem.mNewPath = newKey;
//            ContentValues values = new ContentValues();
//            values.put(ModelColumns.IMAGE_NEW_PATH, imageItem.mNewPath);
//            String where = ModelColumns.IMAGE_NAME + "=? AND " + ModelColumns._ID + "=?";
//            String[] whereArgs = new String[] {imageItem.mImageName, String.valueOf(mID)};
//            mContentResolver.update(ModelColumns.IMAGE_INFO_URI, values, where, whereArgs);
//        }
//    }

    public String getImageQueryByName(String name) {
        return ModelColumns.IMAGE_NAME + "='" + name + "' AND " +
                ModelColumns._ID + "=" + mID;
    }
    ...
    private void saveToDB(ContentValues values) {
        ...
        values.put(ModelColumns.SUPPORT_SCREEN, mSupportScreen);
        values.put(ModelColumns.SCENE_FILE, mSceneFile);
        values.put(ModelColumns.BASEDATA_FILE, mBasedataFile);
        values.put(ModelColumns.ASSETS_PATH, mAssetsPath);
    ...
    public String getSpecialAttribute(Context context, String name) {
        if (spatialAttributes == null) {
            spatialAttributes = HomeUtils.loadSpecialAttributeForModel(context, this);
        }
        if (spatialAttributes != null) {
