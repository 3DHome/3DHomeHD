...
class _SetLightTravel : public SE_SpatialTravel
{
public:
    _SetLightTravel()
    {
    ...
    std::map<std::string,SE_Light*> sceneLights;
    float envBrightness;
...
public:
    ...
    int visit(SE_SimObject* simObject)
    {
        ...
        int num = simObject->getMesh()->getSurfaceNum();
        for(int i = 0; i < num; ++i)
        {
            simObject->getMesh()->getSurface(i)->removeAllLights();
...
bool SE_Scene::_gatherLight(SE_Light *light)
{
    std::string name = light->getLightName();
    std::map<std::string,SE_Light*>::iterator it = mSceneLights.find(name);
...
{    
    ...
    this->sceneApplyLight();
    
}

void SE_Scene::sceneApplyLight()
{
    _SetLightTravel setlight;
    
    setlight.sceneLights = mSceneLights;
    setlight.envBrightness = mLowestBrightness;

    if(mSceneRoot)
    {
        mSceneRoot->travel(&setlight,true);    
    }    
}
...
SE_Light::LightPara* SE_Scene::updateSceneLight(const char* lightName)
{
    std::string name = lightName;
    std::map<std::string,SE_Light*>::iterator it = mSceneLights.find(name);
    if(it == mSceneLights.end())
    ...
    return it->second->getLightPara();
}

void SE_Scene::refreshSceneLightStatus()
{
    //show light has mutex
    sceneApplyLight();
}
