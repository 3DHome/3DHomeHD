...
SE_Spatial* SE_CommonNode::clone(SE_Spatial* parent, int index,bool createNewMesh,const char* statuslist)
{
    ...
    {        
        ...
        sp->clone(dest, index,createNewMesh,(*statusit).c_str());
        statusit++;
...
void SE_CommonNode::createLocalBoundingVolume()
{
    ...
    {
        SE_Spatial* s = *it;
        if(s->getLocalBoundingVolume() == NULL)
        {
            s->createLocalBoundingVolume();
        }
        if(s->getLocalBoundingVolume() != NULL)
        {
            mLocalBoundingVolume->merge(s->getLocalBoundingVolume());
        }
    }
}
...
void SE_CommonNode::setLightness(float lightness)
{
    std::list<SE_Spatial*>::iterator it = mImpl->children.begin();
    for(; it != mImpl->children.end() ; it++)
    {        
        SE_Spatial *sp = *it;        
        sp->setLightness(lightness);
    }
}
