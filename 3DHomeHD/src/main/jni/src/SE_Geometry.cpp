...
void SE_Geometry::writeEffect(SE_BufferOutput& output)
{
    ...
    mspatialData.w = this->getSpatialState();
...
void SE_Geometry::readEffect(SE_BufferInput& input)
{
    ...
    this->setSpatialState(effectData.w);
#endif
        ...
        for(itRU = renderUnitVector.begin() ; itRU!= renderUnitVector.end(); itRU++)
        {
            if(*itRU)
            {
                ...
                //transparency object should be draw after normal object.
                    ...
                    else if((*itRU)->isNeedGenerateMirror())
                    {
                        renderManager->addRenderUnit(*itRU,(*itRU)->getMirrorObject()->getSpatialName());
                    }
...
SE_Spatial *SE_Geometry::clone(SE_Spatial* parent,  int index,bool createNewMesh,const char* statuslist)
{
    ...
    sm->set(dest->getSpatialName(),dest);*/

   /* std::string parentName = parent->getSpatialName();

    if(parentName.find("@") == std::string::npos)
    {
        std::string myName = destobj->getPrefixName();
        if (destobj->getSuffixName())
        {
            myName = myName + "@" + parent->getSpatialName() + "@" + destobj->getSuffixName();
        } else
        {
            myName = myName + "@" + parent->getSpatialName();
        }
        dest->setSpatialName(myName.c_str());
        destobj->setName(myName.c_str());
    }
    else
    {
        dest->setSpatialName(destobj->getName());        
    }*/
...
void SE_Geometry::createLocalBoundingVolume()
{
    if(mLocalBoundingVolume != NULL)
    {
        return;
    }
    mLocalBoundingVolume = new SE_AABBBV;
    SE_Geometry::_Impl::SimObjectList::iterator it;
    SE_Matrix4f localM;
    localM.identity();
    ...
    SE_Matrix4f parentM;
    parentM.set(parent->getLocalRotate().toMatrix3f(), parent->getLocalScale(), parent->getLocalTranslate());
    localM = parentM.mul(localM);
    for(it = mImpl->attachObject.begin() ; it != mImpl->attachObject.end() ; it++)
    {
        ...
        if((*it)->getMesh() == NULL)
            continue;
        SE_GeometryData::transform((*it)->getMesh()->getGeometryData(),localM,&data);
        ...
        LOGI("#### local point num = %d ###\n", num);
        mLocalBoundingVolume->createFromPoints(points, num);
    }
}
...
bool SE_Geometry::applyLight()
{
    std::vector<std::string> lightNameList;
    this->getLightsNameList(lightNameList);
    if(lightNameList.size() == 0)
    {
        return false;
    }

    if(!this->getCurrentAttachedSimObj())
    {
        return false;
    }

    if(!this->getCurrentAttachedSimObj()->getMesh())
    {
        return false;
    }

    int num = this->getCurrentAttachedSimObj()->getMesh()->getSurfaceNum();        

    for(int i = 0; i < num; ++i)
    {
        this->getCurrentAttachedSimObj()->getMesh()->getSurface(i)->removeAllLights();

        for(int j = 0; j < lightNameList.size(); ++j)
        {
            std::string name = lightNameList[j]; 
            SE_Light* l = this->getScene()->getLight(name.c_str());
            if(!l)
            {
                return false;
            }

            float envBrightness = this->getScene()->getLowestEnvBrightness();
            
                this->getCurrentAttachedSimObj()->getSpatial()->setNeedLighting(true);
                this->getCurrentAttachedSimObj()->getMesh()->getSurface(i)->addLight(l,l->getLightType());
                this->getCurrentAttachedSimObj()->getMesh()->getSurface(i)->setEnvBrightness(envBrightness);
...
void SE_Geometry::renderSpatialToTarget(SE_Camera *camera,SE_RenderManager* renderManager, SE_RenderTarget* rt,SE_CULL_TYPE cullType,SE_Spatial* mirror)
{
#if 1
    ...
    for(it = mImpl->attachObject.begin() ; it != mImpl->attachObject.end() ; it++)
    {
        ...
        SE_SimObject::RenderUnitVector renderUnitVector = so->createUnitForTarget(rt);
