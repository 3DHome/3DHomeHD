...
void SE_Renderer::setMatrix(SE_RenderUnit* renderUnit)
{
    ...
    else if(renderUnit->isNeedGenerateMirror())
    {        
    ...
    else if(renderUnit->isNeedGenerateReflection())
    {
...
void SE_Renderer::loadTexture2D(int index, SE_ImageData* imageData, SE_WRAP_TYPE wrapS, SE_WRAP_TYPE wrapT, SE_SAMPLE_TYPE min, SE_SAMPLE_TYPE mag,bool needReGenerateMipmap)
{
    if(imageData == NULL)
    {
...
void SE_Renderer::setImage(int texIndex, SE_RenderUnit* renderUnit)
{
    ...
    else
    {
        ...
        for(int i = 0; i < texUnitNum; ++i)
        {
            ...
            SE_Vector2f userTile = renderUnit->getRenderableSpatial()->getTexCoordXYTile();
            utiling = texUnit->getUTiling() * userTile.x;
            vtiling = texUnit->getVTiling() * userTile.y;
            if(imageDataIDNum > 0)
            {
                ...
                {   
                    ...
                    if(rt)
                    {
                        if(rt->getRenderTargetType() == SE_RenderTarget::TEXTURE)
                        {
                            ...
                            loadTexture2D(replaceIndex,fboTexture,(SE_WRAP_TYPE)colorfbo->getWrapS(),(SE_WRAP_TYPE)colorfbo->getWrapT(),(SE_SAMPLE_TYPE)colorfbo->getSampleMin(),(SE_SAMPLE_TYPE)colorfbo->getSampleMag(),true);
                            hasTexture = true;
                            utiling = 1.0;
                            vtiling = 1.0;  
                        }
...
{
    //not use now,render target will manager view port
    //after 2 or 3 version,this function will be remove
    //glViewport(x, y, w, h);
}
...
{    
    ...
    if(renderUnit->isNeedGenerateMirror())
    {
    ...
    if(renderUnit->getRenderableSpatial())
    {
        float lightness = renderUnit->getRenderableSpatial()->getLightness();
        
        if(((SE_SimpleSurfaceShaderProgram*)mBaseShaderProgram)->getLightnessLoc() != -1)
        {
            glUniform1f(((SE_SimpleSurfaceShaderProgram*)mBaseShaderProgram)->getLightnessLoc(), lightness);
        }
    }
...
{   
    ...
    {
        float* lightdirs = new float[3 * dirlightNum];    
        ...
        if(dirlightNum > 0)
        {
            if(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getDirectionLightDirUniformLoc() != -1)
            {
                glUniform3fv(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getDirectionLightDirUniformLoc(), dirlightNum, lightdirs);
            }

            if(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getLightColorUniformLoc() != -1)
            {
                glUniform3fv(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getLightColorUniformLoc(), dirlightNum, lightcolor);
            }

            if(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getDirLightStrengthUniformLoc() != -1)
            {
                glUniform1fv(((SE_SimpleLightingShaderProgram*)mBaseShaderProgram)->getDirLightStrengthUniformLoc(), dirlightNum, dirlightstrength);
            }
...
#if 0
                //just for test
                if(renderUnit->getNeedHighLight())
                {
                    light->setDirLightStrength(1);
                }
                else
                {
                    light->setDirLightStrength(0.5);
                }
#endif
                SE_Vector3f dir = light->getLightDir();    
                SE_Matrix3f t = w2m.toMatrix3f();
...
void SE_ShadowMapRenderer::setImage(SE_RenderUnit* renderUnit)
{
    ...
    if(!texture)
    {
        bool renderShadow = renderUnit->isNeedRenderShadow();
    ...
    else
    {
        ...
        // one object can just has one effect,now 
        if(renderUnit->isNeedRenderShadow())
        {
            //render shadow to object
            rt = rtm->getShadowTarget();
        }
