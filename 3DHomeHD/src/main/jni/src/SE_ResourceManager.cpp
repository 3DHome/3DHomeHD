...
#ifdef HD3Dhome
    if(spatialType == "SE_Geometry")
    {
    //1.create new transform commonNode
        SE_Spatial* transformCommonNode = new SE_CommonNode();
        SE_CommonNode* t = (SE_CommonNode*)transformCommonNode;
        t->setNodeType(GROUP_NODE);

    //2.set transform commonNode parent is parent        
        transformCommonNode->setParent(parent);
        if(parent)
        {
            parent->addChild(transformCommonNode);
        }

    //3.set spatial parent is transform commonNode
        spatial->setParent(transformCommonNode);
        transformCommonNode->addChild(spatial);
...
SE_Spatial* SE_ResourceManager::createSceneNode(SE_BufferInput& inputBuffer, SE_Spatial* parent,SE_BufferInput* effectBuffer)
{
    ...
    if(spatialType == "SE_Geometry")
    {
        ...
        LOGI("geometry transform node name = %s\n", spatialName.c_str());
        std::string transformNodeName = spatialName + "_transform";
        //std::string transformNodeName = spatialName;
        spatial->getParent()->setSpatialName(transformNodeName.c_str());
        //spatial->setSpatialName(spatialName + "_geometry");
        //5.get transform from spatial
        SE_Vector3f translate = spatial->getLocalTranslate();
        SE_Vector3f scale = spatial->getLocalScale();
        SE_Quat quat = spatial->getLocalRotate();

        //6.set transform to transformNode,and set
        spatial->getParent()->setLocalTranslate(translate);
        spatial->getParent()->setLocalScale(scale);
        spatial->getParent()->setLocalRotate(quat);
...
void SE_ResourceManager::loadPlugins(const char* filePath)
{
    ...
    //SE_File::FindFile(result,"/data/data/com.borqs.se/app_lib","*.so");    
#endif
