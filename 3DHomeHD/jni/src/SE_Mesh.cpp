...
void SE_Surface::getDirLights(std::vector<SE_Light*>& result)
{
    result = mDirLights;
}

void SE_Surface::getPointLights(std::vector<SE_Light*>& result)
{
    result = mPointLights;
}

void SE_Surface::getSpotLights(std::vector<SE_Light*>& result)
{
    result = mSpotLights;
}

void SE_Surface::addLight(SE_Light* light,SE_Light::LightType type)
{
    switch(type)
    {
    case (int)SE_Light::SE_DIRECTION_LIGHT:
        mDirLights.push_back(light);
        break;
    case (int)SE_Light::SE_POINT_LIGHT:
        mPointLights.push_back(light);
        break;
    case (int)SE_Light::SE_SPOT_LIGHT:
...
void SE_Surface::removeAllLights()
{
        mDirLights.clear();
        mPointLights.clear();
        mSpotLights.clear();
...
void SE_Surface::removeLight(SE_Light* light,SE_Light::LightType type)
{
    switch(type)
    {
    case (int)SE_Light::SE_DIRECTION_LIGHT:
        removeElement(mDirLights,light);
        break;
    case (int)SE_Light::SE_POINT_LIGHT:
        removeElement(mPointLights,light);
        break;
    case (int)SE_Light::SE_SPOT_LIGHT:        
        removeElement(mSpotLights,light);
        break;
        
    }
}
void SE_Surface::removeElement(std::vector<SE_Light*> &vec, SE_Light *l)
{
    ...
    if(SE_Application::getInstance()->SEHomeDebug)
    LOGI("light remove fail,not found light named [%s]\n",l->getLightName());
}

int SE_Surface::getTotalLightCount()
{
    return mDirLights.size() + mPointLights.size() + mSpotLights.size();
}

float SE_Surface::getEnvBrightness()
{
    return mEnvBrightness;
}

void SE_Surface::setEnvBrightness(float envbright)
{
    mEnvBrightness = envbright;
}
