...
public:
	...
	*/
     ...
     void getDirLights(std::vector<SE_Light*>& result);
     void getPointLights(std::vector<SE_Light*>& result);
     void getSpotLights(std::vector<SE_Light*>& result);

     void addLight(SE_Light* light,SE_Light::LightType type);

    float getEnvBrightness();

    //set lowest evn brightness from 0.0 to 1.0;
    //0.0 the object back side will be black
    //1.0 the object back side will be texture color.
    //
    void setEnvBrightness(float envlight);
    
    void removeAllLights();
    
    void removeLight(SE_Light* light,SE_Light::LightType type);
...
SE_Vector3f mLightPos;
    ...
    std::vector<SE_Light*> mDirLights;
    std::vector<SE_Light*> mPointLights;
    std::vector<SE_Light*> mSpotLights;
    float mEnvBrightness;
