...
public:
    ...
    void setRenderType(const char* type)
    {
        mRenderType = type;
    }
    ...
    bool needBlending()
    {
	return (mRenderState & BLENDING_MASK) == BLENDING_MASK;
    }

    bool needForeverBlending()
    {
        return (mRenderState & FOREVERBLEND_MASK) == FOREVERBLEND_MASK;
    }

    bool needCullFace()
    {
	return (mRenderState & CULLFACE_MASK) == CULLFACE_MASK;
    }

    bool needDepthTest()
    {
        return (mRenderState & DEPTHTEST_MASK) == DEPTHTEST_MASK;
    }

    bool needAlphaTest()
    {
        return (mRenderState & ALPHATEST_MASK) == ALPHATEST_MASK;
    }
/*
    bool isSelected()
    {
        return (mState & SE_Spatial::SELECTED_MASK) == SE_Spatial::SELECTED; 
    }

    bool canDoCollision()
    {
        return (mState & SE_Spatial::COLLISION_MASK) == SE_Spatial::COLLISIONABLE;
    }

    bool canMove()
    {
        return (mState & SE_Spatial::MOVABILITY_MASK) == SE_Spatial::MOVABLE;
    }*/

    bool isVisible()
    {
        return (mRenderState & VISIBLE_MASK) == VISIBLE_MASK;
    }

    bool isMiniBox()
    {
        return (mRenderState & MINIBOX_MASK) == MINIBOX_MASK;
    }
/*
...
*/
    ...
    enum {BLENDING_MASK = 0x01, DEPTHTEST_MASK = 0x02, CULLFACE_MASK = 0x04, ALPHATEST_MASK = 0x08, VISIBLE_MASK = 0x10, MINIBOX_MASK = 0x20,FOREVERBLEND_MASK = 0x40};
