...
*/
    ...
    for(int i = 0 ; i < meshNum ; i++)
    {
        ...
        //object manager not use
        /*if(!SE_Application::getInstance()->getSimObjectManager()->set(simID, simObj))
        {
            LOGI("[primitive] simObj has exist!!!\n");
            delete simObj;
        }*/
    }
    geometry->setShaderType(mShaderType.c_str());
    geometry->setRenderType(mRenderType.c_str());
    geometry->setSpatialStateAttribute(SE_SpatialAttribute::VISIBLE,isVisible());
    geometry->setLocalTranslate(mLocalTranslate);
    geometry->setLocalRotate(mLocalRotate);
    geometry->setLocalScale(mLocalScale);
    geometry->setUserTranslate(mUserTranslate);
    geometry->setUserRotate(mUserRotate);
    geometry->setUserScale(mUserScale);    
    ...
    SE_BlendState* blendState = (SE_BlendState *)geometry->getRenderState(BLENDSTATE);
    if(!blendState)
    {
        blendState = new SE_BlendState();
        geometry->setRenderState(BLENDSTATE, blendState);     
    }
    geometry->setSpatialEffectAttribute(SE_SpatialAttribute::BLENDABLE,needBlending());
    geometry->setNeedForeverBlend(needForeverBlending());
    //geometry->changeRenderState(1,needBlending());
    SE_DepthTestState* depthTestState = (SE_DepthTestState *)geometry->getRenderState(DEPTHTESTSTATE);
    if(!depthTestState)
    {
        depthTestState = new SE_DepthTestState();
        geometry->setRenderState(DEPTHTESTSTATE, depthTestState);
    }
    geometry->changeRenderState(0,needDepthTest());
    geometry->setSpatialEffectAttribute(SE_SpatialAttribute::CULLFACE,needCullFace());
    geometry->setSpatialEffectAttribute(SE_SpatialAttribute::ALPHATEST,needAlphaTest());
    geometry->setSpatialEffectAttribute(SE_SpatialAttribute::MINIBOX,isMiniBox());
    geometry->updateWorldLayer();
    geometry->getWorldLayer()->setLayer(mLayerIndex);
    if (mObjectType == 1)
    {
        geometry->setNeedDrawLine(true);
