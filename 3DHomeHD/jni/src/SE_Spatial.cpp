...
SE_Spatial::SE_Spatial(SE_Spatial* parent)
{
    ...
    mLocalBoundingVolume = NULL;

mTexCoordTile.set(1,1);
}
...
SE_Spatial::~SE_Spatial() 
{
    ...
    if(mLocalBoundingVolume)
        delete mLocalBoundingVolume;
...
void SE_Spatial::createLocalBoundingVolume()
{
    mLocalBoundingVolume = NULL;
}
void SE_Spatial::clearLocalBoundingVolume()
{
...
void SE_Spatial::updateWorldTransform()
{
    ...
    if(mParent)
    {
        ...
        localM = mPrevMatrix.mul(localM).mul(mPostMatrix).mul(mSpatialFrameMatrix);
    ...
    else
    {
        ...
        {
            mWorldTransform = mPrevMatrix.mul(localM).mul(mPostMatrix).mul(mSpatialFrameMatrix);
        }
...
void SE_Spatial::setSpatialFrameMatrix(const SE_Quat& rotate, const SE_Vector3f& scale, const SE_Vector3f& translate)
{
    mSpatialFrameMatrix.set(rotate.toMatrix3f(), scale, translate);
    setChildrenHasTransform(true);
}
void SE_Spatial::setSpatialFrameMatrix(const SE_Vector4f& col1, const SE_Vector4f& col2, const SE_Vector4f& col3, const SE_Vector4f& col4)
{
    mSpatialFrameMatrix.setColumn(col1, col2, col3, col4);
    setChildrenHasTransform(true);
}
...
void SE_Spatial::read(SE_BufferInput& input)
{
    ...
    mSpatialData.mSpatialState = input.readInt();
...
{}
void SE_Spatial::renderSpatialToTarget(SE_Camera* camera, SE_RenderManager* renderManager,SE_RenderTarget* rt, SE_CULL_TYPE cullType,SE_Spatial* mirror)
{}
...
{}
void SE_Spatial::setLightness(float lightness)
{
