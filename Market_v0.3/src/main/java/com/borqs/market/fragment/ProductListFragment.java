...
//        mPager.getLayoutParams().height = width/2;
        mPagesNumber.removeAllViews();
        recommendPhotoAdapter = new RecommendPhotoAdapter(getFragmentManager(), mRecommends,supported_mod, product_type);
    ...
    private void getProductList() {
        mApiUtil.getProductList(mActivity.getApplicationContext(), mPage, count, product_type,
                MarketConfiguration.VERSION_CODE, MarketConfiguration.PACKAGE_NAME, supported_mod, orderBy, new RequestListener() {
    ...
    private void getRecommendList() {
        ...
        mApiUtil.getRecommendList(mActivity.getApplicationContext(), product_type, new RequestListener() {
                    ...
                    public void onIOException(IOException e) {
                        BLog.d(TAG,
                                "getRecommendList onIOException  "
                                        + e.getMessage());
                        synchronized (mLockedRecommend) {
                    ...
                    public void onComplete(String response) {
                        ...
                        try {
                            ...
                            if (!obj.has("data"))
                                return;
                            rmdList = RecommendJSONImpl
                                    .createRecommendList(obj.optJSONArray("data"));

                            BLog.v(TAG,
                                    "finish getRecommendList="
                                            + rmdList.size());
